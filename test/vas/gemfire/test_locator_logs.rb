...
module Gemfire
  ...
  class TestLocatorLogs < VasTestCase
    ...
    def test_list
      logs = LocatorLogs.new(
          'https://localhost:8443/gemfire/v1/nodes/1/locator-instances/2/logs/',
          StubClient.new)
      assert_count(2, logs)
    ...
    def test_log
      location = 'https://localhost:8443/gemfire/v1/nodes/0/locator-instances/3/logs/4/'
        
      log = LocatorLog.new(location, StubClient.new)
  
      assert_equal('example.log', log.name)
      assert_equal(17638, log.size)
      assert_equal(1337872856, log.last_modified)

      assert_equal('https://localhost:8443/gemfire/v1/nodes/0/locator-instances/3/', log.instance.location)
      assert_equal('https://localhost:8443/vfabric/v1/security/5/', log.security.location)
    end

    def test_content
      log = LocatorLog.new('https://localhost:8443/gemfire/v1/nodes/0/locator-instances/3/logs/4/', StubClient.new)
      content = ""
    ...
    def test_delete
      ...
      logs = LocatorLogs.new('https://localhost:8443/gemfire/v1/nodes/1/locator-instances/2/logs/', client)
      location = 'https://localhost:8443/gemfire/v1/nodes/0/locator-instances/3/logs/4/'
      client.expect(:delete, nil, [location])

      logs.delete(LocatorLog.new(location, client))
