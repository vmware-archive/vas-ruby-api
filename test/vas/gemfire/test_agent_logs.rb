...
module Gemfire
  ...
  class TestAgentLogs < VasTestCase
    ...
    def test_list
      logs = AgentLogs.new(
          'https://localhost:8443/gemfire/v1/nodes/1/agent-instances/2/logs/',
          StubClient.new)
      assert_count(2, logs)
    ...
    def test_log
      location = 'https://localhost:8443/gemfire/v1/nodes/0/agent-instances/3/logs/4/'
      ...
      assert_equal('https://localhost:8443/gemfire/v1/nodes/0/agent-instances/3/', log.instance.location)
      assert_equal('https://localhost:8443/vfabric/v1/security/5/', log.security.location)
    end

    def test_content
      log = AgentLog.new('https://localhost:8443/gemfire/v1/nodes/0/agent-instances/3/logs/4/', StubClient.new)
      content = ""
    ...
    def test_delete
      ...
      logs = AgentLogs.new('https://localhost:8443/gemfire/v1/nodes/1/agent-instances/2/logs/', client)
      location = 'https://localhost:8443/gemfire/v1/nodes/0/agent-instances/3/logs/4/'
      client.expect(:delete, nil, [location])

      logs.delete(AgentLog.new(location, client))
