...
module Gemfire
  ...
  class TestNodes < VasTestCase
    ...
    def test_detail
      ...
      assert_equal('https://localhost:8443/gemfire/v1/groups/2/', node.groups[0].location)
      assert_equal('https://localhost:8443/gemfire/v1/groups/1/', node.groups[1].location)
      assert_equal('https://localhost:8443/vfabric/v1/security/3/', node.security.location)
      assert_equal('https://localhost:8443/gemfire/v1/nodes/0/agent-instances/', node.agent_instances.location)
      assert_equal('https://localhost:8443/gemfire/v1/nodes/0/cache-server-instances/', node.cache_server_instances.location)
