...
module WebServer
  ...
  class TestInstances < VasTestCase
    ...
    def test_instance
      ...
      client.expect(:post, nil, ['https://localhost:8443/web-server/v1/groups/2/instances/4/state/', {:status => 'STARTED'}])
      client.expect(:post, nil, ['https://localhost:8443/web-server/v1/groups/2/instances/4/state/', {:status => 'STOPPED'}])
    ...
    def test_delete
      client = StubClient.new
      instances = Instances.new('https://localhost:8443/web-server/v1/groups/1/instances/', client)
