...
module WebServer
  class TestLogs < VasTestCase
    ...
    def test_list
      logs = Logs.new(
          'https://localhost:8443/web-server/v1/nodes/1/instances/2/logs/',
          StubClient.new)
      assert_count(2, logs)
    ...
    def test_log
      location = 'https://localhost:8443/web-server/v1/nodes/0/instances/3/logs/4/'
        
      log = Log.new(location, StubClient.new)
  
      assert_equal('access_log', log.name)
      assert_equal(149, log.size)
      assert_equal(1337872856, log.last_modified)

      assert_equal('https://localhost:8443/web-server/v1/nodes/0/instances/3/', log.instance.location)
      assert_equal('https://localhost:8443/vfabric/v1/security/5/', log.security.location)
    end

    def test_content
      log = Log.new('https://localhost:8443/web-server/v1/nodes/0/instances/3/logs/4/', StubClient.new)
      content = ""
    ...
    def test_delete
      ...
      logs = Logs.new('https://localhost:8443/web-server/v1/nodes/1/instances/2/logs/', client)
      location = 'https://localhost:8443/web-server/v1/nodes/0/instances/3/logs/4/'
      client.expect(:delete, nil, [location])

      logs.delete(Log.new(location, client))
