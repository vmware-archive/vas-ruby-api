...
module WebServer
  ...
  class TestNodes < VasTestCase
    ...
    def test_list
      nodes = Nodes.new(
          "https://localhost:8443/web-server/v1/nodes/",
          StubClient.new)
    ...
    def test_detail
      ...
      assert_equal('https://localhost:8443/web-server/v1/groups/1/', node.groups[0].location)
      assert_equal('https://localhost:8443/web-server/v1/groups/2/', node.groups[1].location)
